...
- **Improved search visibility (SEO).** Added meta tags, per-page descriptions, and a sitemap. A follow-up audit (mid-July) found and fixed several more issues: a dead link from the site's old address that was sending visitors and search engines to a 404 instead of here; a technical quirk that was confusing Google about which version of some pages to show in search results; missing image descriptions (now added, including real award details verified against The Blue Alliance for all 13 trophy banners); and backwards caching settings that could serve visitors a week-old homepage. Still outstanding: getting the site listed on Team 449's official FIRST profile (flagged for Rafi/James), and adding modern, faster-loading image formats (a larger project for later).
