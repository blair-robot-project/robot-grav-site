...
- **To see what changed and when:** the repo's [commit history](https://github.com/blair-robot-project/robot-grav-site-sync/commits/main) — click any commit for the exact files touched. Commit messages are generated by Claude (a `post-commit` hook, added 2026-07-30) rather than git-sync's generic default — see the 2026-07-30 CHANGELOG entry for the mechanism, the credential (a disposable, spend-limited API key, not a personal one), and a real gotcha specific to live's ownership model (testing the hook must be done as `grav` via `sudo`, not as your own SSH login — running it as the wrong user silently does nothing rather than erroring).
