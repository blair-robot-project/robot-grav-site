...
**Systematic pattern identified, audit started but NOT complete - this is the actual state of things, not "Phase 3 done":** every regression in 11, 12, 13, 14, and 15 above turned out to be the *same underlying gap*, just in a different property each time: **Quark 1's stock `user/themes/quark/css-compiled/theme.min.css` (and to a lesser extent `spectre.min.css`) provides real, load-bearing CSS for classes that Mod Quark's *own* `custom.css` uses but never restates** (because the child theme gets it "for free" via inheritance from the parent). Phase 2's audit only ever diffed Mod Quark's custom.css against itself / against what Quark 2 needed - it had no way to catch a property that live gets from a file the audit never looked at. Finding these one CSS property at a time (as items 11-15 did) clearly isn't converging fast enough - **the next session should do this systematically instead:**

**The task, precisely:** for every CSS selector used in live's `user/themes/mod-quark/css/custom.css`, check whether live's `user/themes/quark/css-compiled/theme.min.css` (stock Quark 1) *also* defines rules for that same selector - and if so, whether `mod-quark-2/css/custom.css` (or Quark 2's own `user/themes/quark2/css/theme.css`) currently provides an equivalent. Anywhere it doesn't, that's a gap of the exact same shape as items 11/14/15. This needs real CSS parsing (selector → property dict), not more one-off `grep`s - a Python script using a CSS parser (check what's available; `tinycss2` isn't confirmed installed, may need `pip install tinycss2` or a hand-rolled regex parser sufficient for these compiled files, which are fairly regular) comparing the two rule-sets is the right shape for this. The six files needed are already known and fetchable via plain `curl` (no auth needed, all public over HTTPS/the tunnel):
...
**Phase 3 is NOT complete.** Nav/hero/pill/typography (items 1-10) are solid, verified repeatedly. Icon-menu/Sponsors grid mechanism (13) is solid, verified by checking all 3 columns' positions. Icon-menu/Sponsors *exact sizing* (11, 12, 14, 15) is close but has a still-unexplained section-height residual, and the systematic audit above hasn't been run yet - there are very likely more instances of this same pattern waiting in other custom modules (footer, galleries, text/year-modules) that haven't been checked at all yet, since all discovery so far has been on the homepage's icon-menu/Sponsors specifically. Do not tell Brad this is "done" again until the systematic audit has actually run and its findings (if any) are fixed and verified with the comparison tool across the full page list.
